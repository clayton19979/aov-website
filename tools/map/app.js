...
function draw() {
  ...
  const visibleStep = state.systems.length > 10000 && state.camera.zoom < 0.8 ? 2 : 1;
  for (let i = 0; i < state.systems.length; i += visibleStep) {
    const system = state.systems[i];
    const p = project(system);
    if (p.x < -4 || p.y < -4 || p.x > rect.width + 4 || p.y > rect.height + 4) continue;
    const depth = (system.y - state.bounds.minY) / Math.max(1, state.bounds.maxY - state.bounds.minY);
    ctx.fillStyle = `rgba(${140 + depth * 90}, ${175 + depth * 55}, 210, ${0.28 + depth * 0.35})`;
    ctx.beginPath();
    ctx.arc(p.x, p.y, state.camera.zoom > 1.4 ? 1.45 : 0.85, 0, Math.PI * 2);
    ctx.fill();
  }
