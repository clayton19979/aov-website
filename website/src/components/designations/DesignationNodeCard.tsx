...
export function DesignationNodeCard({ node }: { node: DesignationNode }) {
  ...
  const detailId = `node-detail-${node.name.replace(/\s+/g, '-').toLowerCase()}`
  ...
  return (
    ...
    >
      <button
        ...
        className="relative w-full text-left p-4 focus:outline-none focus:ring-1 focus:ring-void-teal/30"
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <p
              className="font-mono text-xs text-white/40 leading-relaxed tracking-wide pt-4"
